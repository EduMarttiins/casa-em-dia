...
function activeChild(){try{if(['bernardo','julia','irmaos'].includes(String(selected)))return String(selected)}catch(e){}return 'bernardo'}
function childName(id){try{return state?.children?.find(c=>c.id===id)?.name||(id==='julia'?'Júlia':id==='irmaos'?'Irmãos':'Bernardo')}catch(e){return id==='julia'?'Júlia':id==='irmaos'?'Irmãos':'Bernardo'}}
...
function enhance(){
 ...
 const version=q('.cs63-version',host);if(version)version.textContent='CasaSegura • V67'
}
